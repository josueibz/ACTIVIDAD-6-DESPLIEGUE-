...
#Widget 1:Selectbox
View =st.sidebar.selectbox(label="Pestaña:", options=["Descripción de datos", "Usuarios", "Regresión Lineal"])
...
if View == "Regresión Lineal":
    ...
    #Select box
    variable_y=st.sidebar.selectbox(label="Variables Objetivo (y)", options=lista_num)
    ...
    with Contenedor_A:
        ...
        #Ajustr el modelo con las variables antes declaradas
        ...
        #Corroboramos el coeficiente de correlación
        coef_Correl_simple=np.sqrt(coef_Deter_simple)
        #Mostramos el dataset
        st.write(coef_Correl_simple)

        #Graph 5 scatterplot
        figure5=px.scatter(data_frame=numeric_df, x=y_pred, y=variable_y, title="Modelo Lineal Simple")
    ...
    with Contenedor_B:
        ...
        #Multiselect box
        variables_x=st.sidebar.multiselect(label="Variables independientes del modelo múltiple (x)", options=lista_num, default="botón correcto")
        from sklearn.linear_model import LinearRegression
        ...
        #Graph 6 SCATTERPLOT
        figure6=px.scatter(data_frame=numeric_df, x=y_pred_M, y=variable_y, title="Modelo Lineal Múltiple")
    ...
    with st.container():
            ...
            frecuencias1 = df['Administrador'].value_counts()
            mapeo_numeros = {nombre: i+1 for i, (nombre, count) in enumerate(frecuencias1.items())}
            ...
            frecuencias2 = df['Usuario'].value_counts()
            # 2. Crear un diccionario de mapeo: nombre -> número según frecuencia
            mapeo_numeros = {nombre: i+1 for i, (nombre, count) in enumerate(frecuencias2.items())}
            # 3. Aplicar el reemplazo en la columna
            df['Usuario_cat'] = df['Usuario'].map(mapeo_numeros)
            df['Usuario_cat'] = df['Usuario_cat'].astype(int)

            frecuencias3 = df['mini juego'].value_counts()
            # 2. Crear un diccionario de mapeo: nombre -> número según frecuencia
            mapeo_numeros = {nombre: i+1 for i, (nombre, count) in enumerate(frecuencias3.items())}
            # 3. Aplicar el reemplazo en la columna
            df['minijuego_cat'] = df['mini juego'].map(mapeo_numeros)
            df['minijuego_cat'] = df['minijuego_cat'].astype(int)

            frecuencias4 = df['color presionado'].value_counts()
            # 2. Crear un diccionario de mapeo: nombre -> número según frecuencia
            mapeo_numeros = {nombre: i+1 for i, (nombre, count) in enumerate(frecuencias4.items())}
            # 3. Aplicar el reemplazo en la columna
            df['color_cat'] = df['color presionado'].map(mapeo_numeros)
            df['color_cat'] = df['color_cat'].astype(int)

            frecuencias5 = df['dificultad'].value_counts()
            # 2. Crear un diccionario de mapeo: nombre -> número según frecuencia
            mapeo_numeros = {nombre: i+1 for i, (nombre, count) in enumerate(frecuencias5.items())}
            # 3. Aplicar el reemplazo en la columna
            df['dificultad_cat'] = df['dificultad'].map(mapeo_numeros)
            df['dificultad_cat'] = df['dificultad_cat'].astype(int)

            frecuencias6 = df['Juego'].value_counts()
            # 2. Crear un diccionario de mapeo: nombre -> número según frecuencia
            mapeo_numeros = {nombre: i+1 for i, (nombre, count) in enumerate(frecuencias6.items())}
            # 3. Aplicar el reemplazo en la columna
            df['Juego_cat'] = df['Juego'].map(mapeo_numeros)
            df['Juego_cat'] = df['Juego_cat'].astype(int)

            numeric_df_nuevo=df.select_dtypes(["float", "int"])
            numeric_df=numeric_df_nuevo.fillna(0)
            lista_num=numeric_df_nuevo.columns
